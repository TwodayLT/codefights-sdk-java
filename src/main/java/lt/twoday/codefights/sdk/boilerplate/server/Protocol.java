package lt.twoday.codefights.sdk.boilerplate.server;
...
public class Protocol {
    ...
    public static String serializeMove(Move move) {
        ...
        if (move.getComment() != null && !move.getComment().trim().isEmpty())
    ...
    public static Move parseMove(String input) {
        ...
        while (index < input.length()) {
            ...
            switch (type) {
                case 'a':
                    rez.addAttack(getArea(input, index++));
                    break;
                case 'b':
                    rez.addBlock(getArea(input, index++));
                    break;
                case '.':
                case 'c':
                    rez.setComment(input.substring(index));
    ...
    protected static ServerResponse parse(String line) {
        ...
        while (index < words.length) {
            ...
            else if (OPPONENT_SCORE.equals(firstKeyword))
                result.score2 = Integer.parseInt(nextKeyword);
            else if (ENEMY_MOVE.equals(firstKeyword))
